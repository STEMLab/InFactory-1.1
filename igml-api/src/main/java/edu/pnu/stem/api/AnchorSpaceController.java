...
import edu.pnu.stem.dao.AnchorSpaceDAO;
import edu.pnu.stem.feature.navigation.AnchorSpace;

@RestController
@RequestMapping("/documents/{docId}/anchorspace")
public class AnchorSpaceController {

	@PostMapping(value = "/{id}", produces = "application/json")
	@ResponseStatus(HttpStatus.CREATED)
	public void createAnchorSpace(@PathVariable("id") String id, @RequestBody ObjectNode json,
		...
		try {
			...
			*/
			c = AnchorSpaceDAO.createAnchorSpace(map, parentId, id, name, description, generalGeometry, duality,
					partialBoundedBy, classType, functionType, usageType);
	...
	public void getAnchorSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
							   HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			IndoorGMLMap map 	= Container.getDocument(docId);
			ObjectNode target 	= Convert2Json.convert2JSON(map, AnchorSpaceDAO.readAnchorSpace(map, id));
	...
	public void updateAnchorSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			// String properties = json.get("properties").asText().trim();
			// String duality = null;
			AnchorSpaceDAO.updateAnchorSpace(map, parentId, id, null, null, geom, duality, partialBoundedBy);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
	...
	public void deleteAnchorSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
								  @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			IndoorGMLMap map = Container.getDocument(docId);
			assert map != null;
			AnchorSpaceDAO.deleteAnchorSpace(map, id);
		} catch (NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
		}
