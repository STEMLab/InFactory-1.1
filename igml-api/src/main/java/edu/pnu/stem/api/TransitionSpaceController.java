...
import edu.pnu.stem.dao.TransitionSpaceDAO;

import edu.pnu.stem.feature.navigation.TransitionSpace;

@RestController
@RequestMapping("/documents/{docId}/transitionspace")
public class TransitionSpaceController {

	@PostMapping(value = "/{id}", produces = "application/json")
	@ResponseStatus(HttpStatus.CREATED)
	public void createtransitionSpace(@PathVariable("id") String id, @RequestBody ObjectNode json,
		...
		try {
			...
			*/
			c = TransitionSpaceDAO.createTransitionSpace(map, parentId, id, name, description, generalGeometry, duality, partialBoundedBy, classType, functionType, usageType);
	...
	public void gettransitionSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
								   HttpServletRequest request, HttpServletResponse response) throws IOException {
		try {
			IndoorGMLMap map = Container.getDocument(docId);
			ObjectNode target = Convert2Json.convert2JSON(map, TransitionSpaceDAO.readTransitionSpace(map, id));
	...
	public void updatetransitionSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
		...
		try {
			...
			TransitionSpaceDAO.updateTransitionSpace(map, parentId, id, null, null, geom, duality, partialBoundedBy );
		}
		catch(NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
	...
	public void deletetransitionSpace(@PathVariable("docId") String docId, @PathVariable("id") String id,
									  @RequestBody ObjectNode json, HttpServletRequest request, HttpServletResponse response) {
		try {
			IndoorGMLMap map = Container.getDocument(docId);
			assert map != null;
			TransitionSpaceDAO.deleteTransitionSpace(map, id);
		}
		catch(NullPointerException e) {
			e.printStackTrace();
			throw new UndefinedDocumentException();
