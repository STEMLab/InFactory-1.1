...
/**
 * Outputs the Well-Known Text representation of a {@link Geometry}. The
 * Well-known Text format is defined in the
 * <A HREF="http://www.opengis.org/techno/specs.htm"> OGC Simple Features
 ...
 * output.
 * <p>
 * A non-standard <code>LINEARRING</code> tag is used for LinearRings. The SFS
 * WKT spec does not define a special tag for <code>LinearRing</code>s. Under
 * it, rings are output using <code>LINESTRING</code>.
 *
 * @version 1.7
 * @see WKTReader
 */
public class WKTWriter3D {
	/**
	 ...
	 * dimension (2 or 3). If the specified output dimension is 3, the Z value of
	 ...
	 */
	public WKTWriter3D(int outputDimension) {
	 ...
	 * If the provided coordinate number is <= 0, coordinates will be written all on
	 * one line.
	 *
	 * @param coordsPerLine
	...
	private void appendGeometryTaggedText(Geometry geometry, int level, Writer writer) throws IOException {
		...
		if (geometry instanceof Point) {
			...
			appendSolidTaggedText((Solid) geometry, level, writer);
		} else {
	...
	private void appendSolidTaggedText(Solid solid, int level, Writer writer) throws IOException {
		writer.write("SOLID ");
		appendSolidText(solid, level, writer);
	}
	...
	/**
	 * Converts a <code>Coordinate</code> to <code>&lt;Point&gt;</code> format, then
	 * appends it to the writer.
	 *
	 * @param coordinate
	 *            the <code>Coordinate</code> to process
	...
	private void appendSolidText(Solid solid, int level, Writer writer) throws IOException {
		if (solid.isEmpty()) {
			writer.write("EMPTY");
		} else {
			int level2 = level;
			...
			writer.write("(");
			level2 = level + 1;
			appendMultiPolygonText(solid.getShell(), level2, writer);
			doIndent = true;

			for (int i = 0; i < solid.getHoles().length; i++) {
				writer.write(", ");
				level2 = level + 1;
				appendMultiPolygonText(solid.getHoles()[i], level2, writer);
			}
			writer.write(")");
